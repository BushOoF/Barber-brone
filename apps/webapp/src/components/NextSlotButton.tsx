...
export function NextSlotButton({ slotIso, loading, onClick }: Props) {
  ...
  return (
    <motion.button
      ...
      className={[
        "relative w-full overflow-hidden rounded-3xl px-6 py-7 text-left text-tg-buttonText",
    ...
    >
      ...
      <div className="relative">
        <div className="eyebrow opacity-90">{t("landing.next_kicker")}</div>
        <div className="mt-2 flex items-baseline gap-2">
          <span className="text-5xl font-extrabold tabular-nums leading-none">
            {loading ? "…" : slotIso ? formatTime(slotIso) : "—"}
          </span>
        </div>
        <div className="mt-2 inline-flex items-center gap-2 rounded-full bg-white/20 px-3 py-1 eyebrow">
